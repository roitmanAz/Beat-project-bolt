...
function StepCard({ step }: { step: (typeof STEPS)[number] }) {
  ...
  const { Icon } = step;
  return (
    <div {...handlers} className="relative rounded-2xl p-8 group overflow-hidden" style={{ ...style, background: "#fff", border: tilt.on ? "1.5px solid rgba(0,200,232,0.4)" : "1px solid rgba(0,200,232,0.14)", boxShadow: tilt.on ? "0 14px 40px rgba(5,15,31,0.12)" : "0 2px 14px rgba(5,15,31,0.07)" }}>
      ...
      <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-6 transition-all duration-300 group-hover:shadow-neon-sm" style={{ background: "rgba(0,200,232,0.1)", border: "1px solid rgba(0,200,232,0.3)" }}>
        <Icon size={24} style={{ color: "#0098b8" }} />
      </div>
      <h3 className="font-display font-semibold text-base mb-3 tracking-wide" style={{ color: "#071428" }}>{step.title}</h3>
      <p className="font-body text-sm leading-relaxed mb-5" style={{ color: "#5a7a9a" }}>{step.desc}</p>
    ...
    <div className="p-10 flex flex-col items-center justify-center">
      <div
        ...
        style={{
          ...
          background: hover ? "rgba(0,200,232,0.07)" : "#f8fbfe",
      ...
      >
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300" style={{ background: hover ? "rgba(0,200,232,0.18)" : "rgba(0,200,232,0.1)", border: "1px solid rgba(0,200,232,0.35)", transform: hover ? "scale(1.08)" : "scale(1)", boxShadow: hover ? "0 0 16px rgba(0,200,232,0.3)" : "none" }}>
          <Upload size={28} style={{ color: "#0098b8" }} />
        </div>
        <div>
          <p className="font-display font-semibold tracking-wide mb-1" style={{ color: "#071428" }}>גרור את קובץ ה-.inf שלך לכאן</p>
      ...
      <p className="font-mono text-xs mt-4 text-center tracking-wider" style={{ color: "#b0bcc8" }}>התשלום מתבצע לאחר בדיקת הקובץ ואישור הצוות</p>
    </div>
  );
}
...
export function CustomService() {
  ...
  return (
    <section id="custom" className="py-24 relative overflow-hidden" style={{ background: "#fff" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={ref} className="reveal text-center mb-16">
          <p className="font-mono text-xs tracking-[0.3em] uppercase mb-3" style={{ color: "#0098b8" }}>// התאמה אישית</p>
          <h2 className="font-display font-bold text-4xl sm:text-5xl mb-4" style={{ color: "#071428" }}>מקצבים שמותאמים בדיוק בשבילך</h2>
          <p className="font-body text-lg max-w-2xl mx-auto" style={{ color: "#5a7a9a" }}>שלח לנו את קובץ ההגדרות של האורגן שלך, והצוות שלנו ייצר עבורך חבילת מקצבים מותאמת אישית — מוכנה להתקנה.</p>
        </div>
        ...
        <div className="rounded-3xl overflow-hidden" style={{ background: "#fff", border: "1px solid rgba(0,200,232,0.18)", boxShadow: "0 4px 30px rgba(5,15,31,0.08)" }}>
          <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="p-10 border-b lg:border-b-0 lg:border-l" style={{ borderColor: "rgba(0,200,232,0.14)" }}>
              <h3 className="font-display font-bold text-2xl mb-2" style={{ color: "#071428" }}>מה כלול בשירות?</h3>
                ...
                {INCLUDES.map((item) => (
                  <li key={item} className="flex items-center gap-3">
                    <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(0,200,232,0.12)", border: "1px solid rgba(0,200,232,0.35)" }}>
                      <Check size={12} style={{ color: "#0098b8" }} />
                    </div>
                    <span className="font-body text-sm" style={{ color: "#2a4060" }}>{item}</span>
                  </li>
                ))}
              </ul>
              <div className="flex items-end gap-3 mb-1">
                <span className="font-mono text-sm line-through" style={{ color: "#b0bcc8" }}>₪299</span>
                <span className="font-display font-black text-4xl" style={{ color: "#00c8e8", textShadow: "0 2px 14px rgba(0,200,232,0.35)" }}>₪199</span>
              </div>
              <p className="font-mono text-xs tracking-wider" style={{ color: "#a0b8c8" }}>לחבילה מלאה · מחיר השקה</p>
            </div>
            <UploadForm />
          </div>
        </div>
